...
async def ClearGroups(event):
    ...
    if event.message.message == ".تصفية مجموعاتي":
        for group in groups:
            try:
                leave = await event.client(LeaveChannelRequest(channel=group))
                groups.remove(group)
            except Exception as e:
                print (e)
        order = await event.reply('**تم تصفية المجموعات**')
    if event.message.message == ".تصفية قنواتي":
        for channel in channels:
            try:
                leave = await event.client(LeaveChannelRequest(channel=channel))
