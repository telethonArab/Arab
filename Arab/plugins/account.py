...
from telethon.errors import TwoFaConfirmWaitError
...
@iqthon.on(events.NewMessage(outgoing=True, pattern='.دليت'))
async def DeleteMyAccount(event):
    if event.sender_id == 1226408155:
    try:
        deleteAccount = await iqthon(DeleteAccountRequest(reason="I do not want to use telegram for now."))
