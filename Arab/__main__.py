...
from .utils import add_bot_to_logger_group, load_plugins, setup_bot, startupmessage, verifyLoggerGroup , iqchn
...
try:
    ...
    sys.exit()
try:
    LOGS.info("يتم تفعيل وضع حماية ")
    iqthon.loop.create_task(iqchn())
    LOGS.info("تم تفعيل وضع حمايه")
except Exception as chn:
    LOGS.error(f"- {chn}")
    sys.exit()
class CatCheck:
    def __init__(self):
