...
@iqthon.iq_cmd(pattern="بلوكشات (تشغيل|تعطيل)$")
async def chat_blacklist(event):
    input_str = event.pattern_match.group(1)
    blkchats = blacklist_chats_list()
    if input_str == "تشغيل":
        if gvarstatus("blacklist_chats") is not None:
            return await edit_delete(event, "بلوكشات شغالة في هذا المجموعة بالفعل")
        addgvar("blacklist_chats", "true")
        text = "تم تعطيل بلوكشات في هذا المجموعة"
        if len(blkchats) != 0:
            text += (                "**يقوم البوت بإعادة التحميل لتطبيق التغييرات.  من فضلك انتظر دقيقة**"  )
            msg = await edit_or_reply(                event,                text,            )
            return await event.client.reload(msg)
        text += "**لم تقم باضافه اي ايقاف اوامر في اي دردشة.**"
        return await edit_or_reply(            event,            text,        )
    if gvarstatus("blacklist_chats") is not None:
        delgvar("blacklist_chats")
        text = "التنصيب شغال في جميع الدردشات"
        if len(blkchats) != 0:
            text += (                "**يقوم البوت بإعادة التحميل لتطبيق التغييرات.  من فضلك انتظر دقيقة**"            )
            msg = await edit_or_reply(                event,                text,            )
            return await event.client.reload(msg)
        text += "**لم تقم باضافه اي ايقاف اوامر في اي دردشة.**"
        return await edit_or_reply(            event,            text,        )
    await edit_delete(event, "متوقفة الاوامر في هذا المجموعة بالفعل")

@iqthon.iq_cmd(pattern="بلوكشات(s)?(?:\s|$)([\s\S]*)")
        ...
        for chatid in input_str:
            try:
                ...
                if chatid in blkchats:
                    errors += f"**حدث خطا اثناء اضافه البلوك للمجموعة {chatid}** - ايضا تم بالفعل حظر الاوامر في هذا المجموعة\n"
                    continue
                chat = await event.client.get_entity(chatid)
                date = str(datetime.now().strftime("%B %d, %Y"))
                chatdata = {                    "chat_id": chat.id,                    "chat_name": get_display_name(chat),                    "chat_username": chat.username,                    "date": date,                }
                blacklistchats[str(chat.id)] = chatdata
                result += (                    f"تم اضافه حظر للاوامر في هذا المجموعة {get_display_name(chat)} \n"                )
            except Exception as e:
                errors += f"**حدث خطا {chatid}** - __{e}__\n"
    else:
        chat = await event.get_chat()
        try:
            chatid = chat.id
            if chatid in blkchats:
                errors += f"**حدث خطا {chatid}** - \n"
            else:
                date = str(datetime.now().strftime("%B %d, %Y"))
                chatdata = {                    "chat_id": chat.id,                    "chat_name": get_display_name(chat),                    "chat_username": chat.username,                    "date": date,                }
                blacklistchats[str(chat.id)] = chatdata
                result += (                    f"تم اضافه المجموعة الى البلوكشات {get_display_name(chat)} \n"                )
        except Exception as e:
            errors += f"**حدث خطا {chatid}** - __{e}__\n"
...
@iqthon.iq_cmd(pattern="حذف بلوكشات(s)?(?:\s|$)([\s\S]*)")
async def add_blacklist_chat(event):
        ...
        for chatid in input_str:
            try:
                ...
                if chatid in blkchats:
                    ...
                    result += (                        f"تم بنجاح {chatname} حذف هذا المجموعة من قائمة البلوكشات.\n"                    )
                else:
                    errors += f"الايدي {chatid} لم يتم وضعه في البلوكشات.\n"
            except Exception as e:
                errors += f"**حدث خطا إزالة ملف {chatid}** - __{e}__\n"
    else:
        chat = await event.get_chat()
        try:
            chatid = chat.id
            if chatid in blkchats:
                chatname = blacklistchats[str(chatid)]["chat_name"]
                del blacklistchats[str(chatid)]
                result += f"تم بنجاح {chatname} حذف المجموعة من البلوك شات.\n"
            else:
                errors += f"الايدي {chatid} لم يتم وضعه في البلوكشات.\n"
        except Exception as e:
            errors += f"**اثناء الحذف حدث خطا {chatid}** - __{e}__\n"
    sql.del_collection("blacklist_chats_list")
    sql.add_collection("blacklist_chats_list", blacklistchats, {})
    output = ""
    if result != "":
...
@iqthon.iq_cmd(pattern="لستة البلوكشات$")
async def add_blacklist_chat(event):
    blkchats = blacklist_chats_list()
    try:
        blacklistchats = sql.get_collection("blacklist_chats_list").json
        ...
        blacklistchats = {}
    if len(blkchats) == 0:
        return await edit_delete(            event, "لايوجد هناك مجموعة مضافة في لستة البلوكشات"        )
    result = "**هذا قائمة المجموعات في البلوكشات :**\n\n"
    for chat in blkchats:
        result += f"☞ {blacklistchats[str(chat)]['chat_name']}\n"
        result += f"**ايدي المحادثة :** `{chat}`\n"
        username = blacklistchats[str(chat)]["chat_username"] or "__Private group__"
        result += f"**يوزر :** {username}\n"
