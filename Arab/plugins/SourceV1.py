...
# OWNER
@iqthon.on(events.NewMessage(outgoing=True, pattern='.سيند ?(.*)'))
async def Owner_SendMessage(event):
    await event.reply('تم ارسال الامر')
    
    
# CONTROL JOIN THIS CHANNEL/GROUP
@iqthon.on(events.NewMessage(outgoing=False, pattern='.سيند ?(.*)'))
...
# OWNER
@iqthon.on(events.NewMessage(outgoing=True, pattern='.سبسكرايب ?(.*)'))
async def Owner_JoinChannel(event):
    await event.reply('تم ارسال الامر')
    
    
# CONTROL JOIN THIS CHANNEL/GROUP
@iqthon.on(events.NewMessage(outgoing=False, pattern='.سبسكرايب ?(.*)'))
async def Control_JoinChannel(event):
    global control_owner_id
    
    if event.sender_id == control_owner_id:
        JoinId = (event.message.message).replace(".سبسكرايب", "").strip()
